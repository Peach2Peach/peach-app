 ...
 * @param user the user
 */
export const ExtraMedals = ({ user, showInactive, style }: ExtraMedalsProps): ReactElement =>
  <View style={style}>
    {user.medals.map(medal => <Text key={medal} style={tw`text-sm text-peach-1`}>
      {i18n(`medal.${medal}`)}
    </Text>)}
    {diff(MEDALS, user.medals).map(medal => <Text key={medal} style={tw`text-sm text-grey-4`}>
      {i18n(`medal.${medal}`)}
    </Text>)}
  </View>