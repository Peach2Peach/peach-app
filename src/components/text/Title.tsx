...
export const Title = ({ title, subtitle, help, style }: TitleProps): ReactElement => {
  ...
  return (
    <View style={[tw`flex items-center`, style]}>
      <Text
        style={[tw`font-baloo text-center text-2xl leading-2xl uppercase text-peach-1`, tw.md`text-3xl leading-3xl`]}
      >
        {title}
      </Text>
      {subtitle ? (
        <View style={tw`flex-row justify-center items-center -mt-3`}>
          <Text style={tw`font-lato text-center leading-6 text-grey-2 `}>{i18n(subtitle)}</Text>
          {help ? (
