...
describe('getCountrySelectDrawerOptions', () => {
  ...
  const result = getCountrySelectDrawerOptions([balticHoneyBadger], goToEventDetails, selectCountry)
  ...
  it('should select country when pressing on country option', () => {
    ...
    expect(selectCountry).toHaveBeenCalledWith({ LV: [balticHoneyBadger] }, 'LV')
  })
})
