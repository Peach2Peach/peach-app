...
export const ConfirmSlider = ({
  ...
  return (
    <View
      style={tw`w-full p-1 overflow-hidden border rounded-5 bg-primary-background-dark-color border-primary-mild-1`}
    ...
    >
      <Animated.View
        ...
        style={[
          ...
          { transform: getTransform(pan, widthToSlide) },
        ]}
      >
        <ConfirmSliderLabel
          style={tw`absolute right-full`}
          width={widthToSlide}
          opacity={pan}
        >
          {label2}
...
export function UnlockedSlider({
  ...
  const { widthToSlide, onLayout } = useConfirmSliderSetup({
    onConfirm: () => null,
    enabled: false,
  });
