...
const accountNumberRules = { required: true, isNUBAN: true }

export const useTemplate21Setup = ({ data, onSubmit, setStepValid, setFormData }: FormProps) => {
  ...
  const buildPaymentData = useCallback(
    ...
    [data?.id, paymentMethod, label, accountNumber, selectedCurrencies],
  )
  ...
  const isFormValid = useCallback(() => {
    ...
    return labelErrors.length === 0 && accountNumberIsValid
  }, [accountNumberIsValid, labelErrors.length, setDisplayLabelErrors])
  ...
  return {
    ...
    accountNumberInputProps: {
      ...
      placeholder: i18n('form.nuban.placeholder'),
