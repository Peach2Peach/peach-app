...
export const Skrill = ({ forwardRef, data, currencies = [], onSubmit, setStepValid }: FormProps): ReactElement => {
  ...
  const buildPaymentData = (): PaymentData & SkrillData => ({
    id: data?.id || `skrill-${new Date().getTime()}`,
    label,
    type: 'skrill',
    email,
    beneficiary,
  ...
  return (
    <>
      ...
      <Input
        ...
        autoCorrect={false}
      />
      <CurrencySelection paymentMethod="skrill" selectedCurrencies={selectedCurrencies} onToggle={onCurrencyToggle} />
    </>
  )
}
