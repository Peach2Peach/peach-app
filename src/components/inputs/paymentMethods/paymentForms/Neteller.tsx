import { ReactElement, useCallback, useContext, useEffect, useImperativeHandle, useMemo, useRef, useState } from 'react'
...
export const Neteller = ({ forwardRef, data, currencies = [], onSubmit, setStepValid }: FormProps): ReactElement => {
  ...
  const buildPaymentData = (): PaymentData & NetellerData => ({
    id: data?.id || `neteller-${new Date().getTime()}`,
    label,
    type: 'neteller',
    email,
    beneficiary,
  ...
  return (
    <>
      ...
      <Input
        ...
        autoCorrect={false}
      />
      <CurrencySelection paymentMethod="neteller" selectedCurrencies={selectedCurrencies} onToggle={onCurrencyToggle} />
    </>
  )
}
