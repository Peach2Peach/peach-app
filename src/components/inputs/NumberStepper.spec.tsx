...
describe("NumberStepper", () => {
  ...
  it("renders correctly with style", () => {
    renderer.render(
      <NumberStepper value={4} onChange={jest.fn()} style={tw`mt-4`} />,
    );
    expect(renderer.getRenderOutput()).toMatchSnapshot();
  });
  ...
  it("allows increasing and decreasing number", () => {
    ...
    const { getByAccessibilityHint } = render(
      <NumberStepper value={defaultValue} onChange={onChange} />,
    );
  ...
  it("prevents increasing and decreasing number when min/max are reached", () => {
    const onChange = jest.fn();
    const { getByAccessibilityHint } = render(
      <NumberStepper value={4} min={4} max={4} onChange={onChange} />,
    );
    fireEvent.press(getByAccessibilityHint("decrease number"));
