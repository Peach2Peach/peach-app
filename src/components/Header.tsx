    ...
    <Shadow {...shadowProps}>
      <View style={tw`w-full flex-row items-center justify-between px-3 py-2 bg-white-1`}>
        <View>
          ...
          <Text style={tw`font-lato text-lg leading-5 text-peach-1 text-right`}>
            {i18n(`currency.format.${bitcoinContext.currency}`, thousands(Math.round(bitcoinContext.price)))}
          </Text>
        </View>
        <View>
          <Text style={tw`font-lato text-lg leading-5 text-grey-1`}>
            {i18n(`currency.${bitcoinContext.currency}`)}
          </Text>
          <Text style={tw`font-lato text-lg leading-5 text-peach-1`}>
