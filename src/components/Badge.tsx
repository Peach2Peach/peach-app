...
export function RepeatTraderBadge ({ id }: { id: User['id'] }) {
  ...
  return (
    <InfoContainer
      ...
      icon={<RepeatTraderIcon positive={!hadBadExperience} numberOfTrades={data.trades} />}
      color={colorTheme}
...
function RepeatTraderIcon ({ positive, numberOfTrades }: { positive: boolean; numberOfTrades: number }) {
  ...
  if (positive) {
    return (
      ...
      >
        <FixedHeightText height={6} style={[tw`subtitle-2 text-10px`, { color: colorTheme }]}>
          {numberOfTrades}
        </FixedHeightText>
      </View>
    )
