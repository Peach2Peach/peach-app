...
describe('useSyncUserAccount', () => {
  ...
  it('should not do anything if local key is already published', () => {
    useSelfUserMock.mockReturnValue({ user: { ...defaultUser, pgpPublicKeys: [account1.pgp] } })
    const { result } = renderHook(useSyncUserAccount)
    expect(result.current).toBeFalsy()
    expect(publishPGPPublicKeyMock).not.toHaveBeenCalled()
  })
  it('should send unpublished keys to server', () => {
    useSelfUserMock.mockReturnValue({ user: { ...defaultUser, pgpPublicKeys: [{ publicKey: 'differentKey' }] } })
    const { result } = renderHook(useSyncUserAccount)
    expect(result.current).toBeTruthy()
    expect(publishPGPPublicKeyMock).toHaveBeenCalledWith(account1.pgp, 0, [account1.pgp])
  })
})
