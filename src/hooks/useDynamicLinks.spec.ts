...
jest
  ...
  .mockImplementation((type, cb: (event: { url: string }) => void) => {
    onLinkHandlers.push(cb);
    return { remove: removeMock } as unknown as EmitterSubscription;
  });

const dynamicLink =
  "https://peachbitcoin.page.link/?link=https%3A%2F%2Fpeachbitcoin.com%2Freferral%3Fcode%3DSATOSHI";
const getInitialURLSpy = jest
  .spyOn(Linking, "getInitialURL")
  .mockResolvedValue(dynamicLink);
jest.useFakeTimers();
