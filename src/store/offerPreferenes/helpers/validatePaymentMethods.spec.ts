...
describe('validatePaymentMethods', () => {
  ...
  it('should return false if no means of payment have been configured', () => {
    expect(validatePaymentMethods({ meansOfPayment: {}, originalPaymentData })).toBe(false)
  })
  ...
  it('should return false if some payment data is invalid', () => {
    expect(
      validatePaymentMethods({
        ...
        originalPaymentData: [
          ...originalPaymentData,
          { id: 'sepa-1234', label: 'SEPA', type: 'sepa', currencies: ['EUR', 'CHF'] },
        ],
      }),
    ).toBe(false)
  })
  ...
  it('should return true if all payment data is valid', () => {
    expect(validatePaymentMethods({ meansOfPayment, originalPaymentData })).toBe(true)
  })
})
