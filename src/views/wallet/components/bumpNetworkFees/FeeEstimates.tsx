...
export const FeeEstimates = ({ estimatedFees, setFeeRate, isOverpaying }: Props) => (
  <View style={tw`flex-row items-center justify-between gap-6 px-2`}>
    ...
    <TouchableOpacity onPress={() => setFeeRate(estimatedFees.halfHourFee.toString())}>
      <FeeInfo label={i18n('wallet.bumpNetworkFees.estimated.halfHourFee')} fee={estimatedFees.halfHourFee} />
    </TouchableOpacity>
    <TouchableOpacity onPress={() => setFeeRate(estimatedFees.hourFee.toString())}>
      <FeeInfo label={i18n('wallet.bumpNetworkFees.estimated.hourFee')} fee={estimatedFees.hourFee} />
    </TouchableOpacity>
  </View>
)
