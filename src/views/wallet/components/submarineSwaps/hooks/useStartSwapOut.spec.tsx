...
const useSubmarineSwapsMock = jest.requireMock('../../../../../utils/boltz/query/useSubmarineSwaps').useSubmarineSwaps.mockReturnValue({
...
describe('useStartSwapOut', () => {
  ...
  it('should throw an error if liquid wallet is not ready', async () => {
    ...
    const { result: { current: startSwapOut } } = renderHook(useStartSwapOut)
  ...
  it('should throw an error if L-BTC <> BTC Pair is not available', async () => {
    useSubmarineSwapsMock.mockReturnValueOnce({ submarineList: { 'L-BTC': {} } })
    const { result: { current: startSwapOut } } = renderHook(useStartSwapOut)
    const error = await getError<Error>(() => startSwapOut());
  ...
  it('should show error banner if funds are insufficient for swap', () => {
    ...
    const { result: { current: startSwapOut } } = renderHook(useStartSwapOut)
    startSwapOut()
    expect(mockShowErrorBanner).toHaveBeenCalledWith("INSUFFICIENT_FUNDS", ["10000", "9999"]);
  })
  it('should handle transaction error if swap estimation fails', () => {
    useLiquidWalletState.getState().setBalance(minimum)
    const { result: { current: startSwapOut } } = renderHook(useStartSwapOut)
    startSwapOut()
    expect(mockHandleTransactionError).toHaveBeenCalledWith([new Error('No input #0'), undefined]);
  })
