...
describe("useBumpFees", () => {
  ...
  beforeEach(() => {
    const transactions = [
      {
        ...pending1,
        txid: bitcoinJSTransactionWithRBF1.getId(),
        sent: 100000,
        received: 20000,
      },
    ];
  ...
  it("should not show bump fee confirmation popup if transaction details could not be loaded", async () => {
    const { result } = renderHook(useBumpFees, {
      initialProps: { ...initialProps, transaction: undefined },
    });
  ...
  it("should broadcast bump fee transaction", async () => {
    ...
    await waitFor(() => {
      ...
      expect(replaceMock).toHaveBeenCalledWith("transactionDetails", {
        txId: newTxId,
      });
    });
  });
