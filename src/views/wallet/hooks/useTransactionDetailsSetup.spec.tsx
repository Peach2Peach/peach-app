...
describe('useTransactionDetailsSetup', () => {
  ...
  it('should return local transaction', async () => {
    ...
    const { result } = renderHook(useTransactionDetailsSetup)
    expect(result.current.transactionSummary).toEqual(transactionWithRBF1Summary)
    await waitFor(() => expect(result.current.transactionDetails).toEqual(bitcoinJSTransactionWithRBF1))
  })
})
