...
export const ReceiveBitcoinLiquid = () => {
  const [index, setIndex] = useState<number>();

  const displayIndex = index ?? peachLiquidWallet?.addresses.length ?? 0;
...
function AddressQRCode({ index }: { index: number }) {
  ...
  if (!peachLiquidWallet) return <Loading />;
  const { address } = peachLiquidWallet?.getAddress(index, true);
  const used = peachLiquidWallet?.isAddressUsed(address);
  return (
    <>
      <QRCode
        value={address}
...
function BitcoinAddress({ index }: { index: number }) {
  if (!peachLiquidWallet) return <Loading />;
  const { address } = peachLiquidWallet?.getAddress(index);
  const isUsed = peachLiquidWallet?.isAddressUsed(address);
