...
describe('OfferPublished', () => {
  it('renders correctly', () => {
    const { toJSON } = render(<OfferPublished offerId={offerId} shouldGoBack={false} />)
    expect(toJSON()).toMatchSnapshot()
  })

  it('should go to search when pressing show offer button', () => {
    const { getByText } = render(<OfferPublished offerId={offerId} shouldGoBack={false} />)
    const button = getByText('show offer')
    fireEvent.press(button)
    expect(navigateMock).toHaveBeenCalledWith('search', { offerId })
  })
