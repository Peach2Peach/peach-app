...
export function CustomAddressScreen({
  ...
  const [addressLabel, setAddressLabel, addressLabelValid, addressLabelErrors] =
    useValidatedState(defaultAddressLabel || "", labelRules);
  const isUpdated =
    address === defaultAddress && addressLabel === defaultAddressLabel;
  const save = () => {
...
function RemoveWalletPopup({
  ...
  const setPayoutAddressSignature = useSettingsStore(
    (state) => state.setPayoutAddressSignature,
  );
  ...
  const removeWallet = () => {
    ...
    if (isPayout) {
      setPayoutAddressSignature(undefined);
