...
const createBlockchainBlockHashMock = (validType?: BlockChainNames) => (config: unknown, type: string) => {
...
describe('checkNodeConnection', () => {
  it('should check electrum connection with success and return electrum node type', async () => {
    ...
    expect((await checkNodeConnection('electrum.node')).result).toBe(BlockChainNames.Electrum)
  ...
  it('should check esplora connection with success and return esplora node type', async () => {
    ...
    expect((await checkNodeConnection('esplora.node')).result).toBe(BlockChainNames.Esplora)
    expect(blockChainCreateMock).toHaveBeenCalledWith(
  ...
  it('should return all errors if no connection could be established', async () => {
    ...
    expect((await checkNodeConnection('esplora.node')).error).toEqual('error\n\nerror')
  })
})
