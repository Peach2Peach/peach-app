...
export const useOptimisticTxHistoryUpdate = () => {
  const [updateTxOfferMap, addTransaction, getFundMultipleByOfferId] = useWalletState(
    (state) => [state.updateTxOfferMap, state.addTransaction, state.getFundMultipleByOfferId],
    shallow,
  )
