...
describe("getSellerStatusText", () => {
  ...
  it("should return the correct status if the offer was republished", () => {
    ...
    expect(getSellerStatusText(contract, true)).toBe(
      i18n("contract.seller.republished"),
    );
  });
  ...
  it("should return the dispute status text if there is a dispute winner", () => {
    expect(
      getSellerStatusText({ disputeWinner: "buyer" } as Contract, true),
    ).toBe("disputeStatusText");
  });
  it("should return the correct status if republish is available (no dispute)", () => {
    expect(
      getSellerStatusText(
        { tradeStatus: "refundOrReviveRequired" } as Contract,
        true,
      ),
    ).toBe(
  ...
  it("should return the correct status if republish is not available", () => {
    expect(
      getSellerStatusText(
        { tradeStatus: "refundTxSignatureRequired" } as Contract,
        true,
      ),
