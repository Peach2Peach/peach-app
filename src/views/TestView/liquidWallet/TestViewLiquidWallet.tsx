...
export const TestViewLiquidWallet = () => {
  const balance = 0
  const [address, setAddress, , addressErrors] = useValidatedState<string>('', liquidAddressRules)
    ...
    <Screen>
      <PeachScrollView>
        <View style={tw`gap-4`}>
          ...
          {!!txId && (
            <View>
              <PeachText onPress={() => showLiquidTransaction(txId, NETWORK)}>txId: {txId}</PeachText>
            </View>
          )}
