...
export const cancelContractAsSeller = async (contract: Contract) => {
  ...
  if (!result?.success || err)
    return { result: { sellOffer: undefined }, error: err?.error };

  if (!result.psbt) return { result: { sellOffer: undefined } };

  const { result: patchOfferResult, error } = await patchSellOfferWithRefundTx(
    contract,
    result.psbt,
  );
...
async function patchSellOfferWithRefundTx(
  contract: Contract,
  refundPSBT: string,
) {
  ...
  const {
    isValid,
    psbt,
    err: checkRefundPSBTError,
  } = checkRefundPSBT(refundPSBT, sellOffer);

  if (checkRefundPSBTError)
    return { result: { sellOffer }, error: checkRefundPSBTError };
  if (!isValid || !psbt)
    return { result: { sellOffer }, error: "UNKNOWN_ERROR" };
  ...
  if (!patchOfferResult || patchOfferError) {
    return {
      result: { sellOffer },
      error: patchOfferError?.error || "UNKNOWN_ERROR",
    };
  }
