...
export default ({ message, reason, contractId, navigation }: YouGotADisputeProps): ReactElement => {
  ...
  useEffect(() => {
    if (contract) {
      ...
      contract.disputeDate = new Date(Date.now())
      contract.disputeInitiator = contract.seller.id === account.publicKey ? contract.buyer.id : contract.seller.id
      const autogeneratedMessages = initDisputeSystemMessages(chat.id, contract)
      saveChat(contractId, {
