...
export const setMultipleRecipients = async (transaction: TxBuilder, amount: number, addresses: string[]) => {
  ...
  const recipients = await Promise.all(addresses.map(getScriptPubKeyFromAddress))
  await transaction.setRecipients(recipients.map((script) => new ScriptAmount(script, splitAmount)))
}
