...
describe('getTxDefaultLabel', () => {
  ...
  it('returns the default label for a tx with contract', () => {
    ...
    useWalletState.getState().updateTxOfferMap(confirmed1.txid, [offerWithContract.id])
  ...
  it('returns the default label for a tx with multiple offers', () => {
    ...
    useWalletState.getState().updateTxOfferMap(confirmed1.txid, [offerSummary.id, offerSummary2.id])
