...
export class PeachLiquidJSWallet {
  ...
  constructor ({ wallet, network = liquid.networks.liquid, gapLimit = defaultGapLimit }: PeachLiquidJSWalletProps) {
  ...
  signMessage (message: string, address: string, index?: number) {
    ...
    const signature = Signer.sign(keyPair.toWIF(), address, message, this.network)
