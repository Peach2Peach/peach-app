...
import { ElementsValue, Transaction as LiquidTransaction, address as liquidAddress, payments as liquidPayments } from "liquidjs-lib";
...
export const buildTransaction = ({ recipient, amount, miningFees = 1, inputs }: BuildTransactionProps): LiquidTransaction => {
  if (!peachLiquidWallet) throw Error('WALLET_NOT_READY');

  const available = inputs.map(utxo => utxo.value).reduce(sum, 0);
  if (available < amount) throw Error(`InsufficientFunds: Insufficient funds: ${available} sat available of ${amount} sat needed`);
