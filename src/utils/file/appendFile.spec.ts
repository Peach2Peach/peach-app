...
describe('appendFile', () => {
  it('should append content to a file', async () => {
    ...
    expect(RNFS.appendFile).toHaveBeenCalledWith(RNFS.DocumentDirectoryPath + path, content, 'utf8')
  })
  ...
  it('should append content to a file with newline', async () => {
    ...
    expect(wasSuccess).toBe(true)
    expect(RNFS.appendFile).toHaveBeenCalledWith(RNFS.DocumentDirectoryPath + path, '\ntest', 'utf8')
  })
