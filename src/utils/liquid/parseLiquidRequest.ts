...
export const parseLiquidRequest = (
  request = "liquidnetwork:",
  ...
  const address = request.split(":").pop()?.split("?").shift() ?? "";

  if (!isLiquidAddress(address, network)) return {};

  parsedRequest.address = address;
  if (urn.searchParams.get("assetid"))
    parsedRequest.assetId = urn.searchParams.get("assetid") || undefined;
  ...
  if (urn.searchParams.get("label"))
    parsedRequest.label = urn.searchParams.get("label") || "";
