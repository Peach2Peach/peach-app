...
jest.mock('react-native-device-info', () => ({
  ...
  getInstallerPackageNameSync: (...args: unknown[]) => getInstallerPackageNameSyncMock(...args),
}))
...
describe('linkToAppStoreAndroid', () => {
  ...
  it('opens the correct URL when installed from Google Play', async () => {
    ...
    expect(openURLSpy).toHaveBeenCalledWith(`https://play.google.com/store/apps/details?id=${bundleId}`)
  })

  it('opens the correct URL when not installed via APK', async () => {
    getInstallerPackageNameSyncMock.mockReturnValueOnce('com.example.installer')
    await linkToAppStoreAndroid()
