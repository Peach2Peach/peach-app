...
describe('checkNotificationStatusAndroid', () => {
  const checkNotifications = jest.spyOn(reactNativePermissions, 'checkNotifications')
  it('returns true if notifications are enabled', async () => {
    checkNotifications.mockResolvedValueOnce({ status: 'granted', settings: { alert: true } })
  ...
  it('returns false if notifications are unavailable', async () => {
    checkNotifications.mockResolvedValueOnce({ status: 'unavailable', settings: { alert: true } })
  ...
  it('returns false if notifications are denied', async () => {
    checkNotifications.mockResolvedValueOnce({ status: 'denied', settings: { alert: true } })
  ...
  it('returns false if notifications are blocked', async () => {
    checkNotifications.mockResolvedValueOnce({ status: 'blocked', settings: { alert: true } })
  ...
  it('returns false if notifications are limited', async () => {
    checkNotifications.mockResolvedValueOnce({ status: 'limited', settings: { alert: true } })
