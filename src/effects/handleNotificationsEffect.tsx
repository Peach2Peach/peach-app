...
export default ({
  ...
  const unsubscribe = messaging().onMessage(async (remoteMessage): Promise<null|void> => {
    ...
    if (offer && type === 'offer.expired'
      && !/contract/u.test(currentPage)) {
      return updateOverlay({
        content: <OfferExpired
          offer={offer as SellOffer}
          days={args ? args[0] || '15' : '15'} navigation={navigationRef}
        />,
      })
    }
    if (offer && type === 'offer.notFunded'
      && !/sell|contract/u.test(currentPage)) {
      return updateOverlay({
        content: <OfferNotFunded
          offer={offer as SellOffer}
          days={args ? args[0] || '7' : '7'} navigation={navigationRef}
        />,
      })
    }
    ...
    if (type === 'contract.contractCreated'
      && !/contract|search/u.test(currentPage)) {
      return updateOverlay({
