import { getTradingPartner } from '../../../../src/utils/contract'
...
describe('getTradingPartner', () => {
  it('returns seller if user is buyer', () => {
    ...
    expect(getTradingPartner(contract as Contract, account as Account)).toBe(seller)
  })
  ...
  it('returns buyer if user is seller', () => {
    ...
    const buyer: Partial<User> = { id: '03abc' }
    const contract: Partial<Contract> = { seller: seller as User, buyer: buyer as User }
    expect(getTradingPartner(contract as Contract, account as Account)).toBe(buyer)
  })
})
